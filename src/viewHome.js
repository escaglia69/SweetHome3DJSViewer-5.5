...
function viewHomeInOverlay(homeUrl, params) {
  ...
  if (params 
      && (params.aerialViewButtonText && params.virtualVisitButtonText 
          || params.viewerControlsAdditionalHTML || params.checkShutters)) {
    ...
    if( typeof params.checkShutters !== 'undefined' )
    {
      divHTML +=
            '   <input  id="checkShutters" class="viewerComponent" name="checkShutters" type="checkbox" style="visibility: hidden;"/>'
          + '     <label class="viewerComponent" for="checkShutters" style="visibility: hidden;">Check Shutters</label>';
    }
    ...
    if (params.aerialViewButtonText && params.virtualVisitButtonText) {
      canvas.homePreviewComponent = new homePreviewComponentContructor(
          ...
          {roundsPerMinute : params.roundsPerMinute, 
           ...
           checkShutters : params.checkShutters});
    } else {
      canvas.homePreviewComponent = new homePreviewComponentContructor(
          "viewerCanvas", homeUrl, onerror, onprogression, 
          {roundsPerMinute : params.roundsPerMinute,
...
function sensorFeed(jsonUrl, home, checkShutters) {
  console.log("checkShutters: "+checkShutters);
  ...
  try {
    ...
    for (var i = 0; i < response.Sensor.length; i++) {
        var sensor = response.Sensor[i];
        console.log(sensor.Id+" "+sensor.Temp+" "+sensor["Int door"]+" "+sensor["Ext door"]+" "+sensor["Battery V"]+" "+sensor["Read time"]);
    }
    //document.writeln("Started at: "+response["Started at"]);
    var furniture = home.getFurniture();
    for (var i = 0; i < furniture.length; i++) {
      var piece = furniture [i];
      if (piece.isDoorOrWindow()) {
        if (piece.getName().match("(D|W)\\d+")) {
          id = parseInt(piece.getName().substring(1));
          if (response.Sensor[id]["Int door"]==1) {
      ...
      if (document.getElementById("checkShutters").checked) {
        if (piece.getName().match("(S)\\d+(A)")) {
          id = parseInt(piece.getName().substring(1,piece.getName().length-1));
          if (null == piece.getDescription()) {
            piece.setDescription("open");
          }
          if (response.Sensor[id]["Ext door"]==0) {
            if (piece.getDescription()=="closed") {
              piece.setX(piece.getX()-piece.getWidth());
              piece.setDescription("open");
            }
          } else {
            if (!(piece.getDescription()=="closed")) {
              piece.setX(piece.getX()+piece.getWidth());
              piece.setDescription("closed");
            }
          }
        }
        if (piece.getName().match("(S)\\d+(B)")) {
          id = parseInt(piece.getName().substring(1,piece.getName().length-1));
          if (null == piece.getDescription()) {
            piece.setDescription("open");
          }
    ...
    if(exception.name == 'NetworkError'){
      console.log('There was a network error towards: '+jsonUrl);
    }
  }
}
              ...
              if (canvas) {
                ...
                previewComponent.prepareComponent(canvasId, onprogression,
                    params ? {roundsPerMinute : params.roundsPerMinute, 
                              ...
                              checkShutters: params.checkShutters}
                           : undefined);
                sensorFeed('../sensors', home, params.checkShutters);
                myVar = setInterval(sensorFeed, 60000, '../sensors', home, params.checkShutters);
...
HomePreviewComponent.prototype.prepareComponent = function(canvasId, onprogression, params) { 
  ...
  if (params && ( typeof params.checkShutters !== 'undefined' )) {
    var checkShutters = document.getElementById("checkShutters");
    checkShutters.addEventListener("change", 
        function() {

        });
    // Make radio buttons and their label visible
    checkShutters.style.visibility = "visible";
    var makeLabelVisible = function(buttonId) {
        var labels = document.getElementsByTagName("label");
        for (var i = 0; i < labels.length; i++) {
          if (labels [i].getAttribute("for") == buttonId) {
    ...
    makeLabelVisible("checkShutters");
